    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "https://knowledgeable-painted-guarantee.glitch.me/penelitian"
        );
        if (!response.ok) {
  ...
  const handleDeleteData = async (id) => {
    try {
      const deleteResponse = await fetch(
        `https://knowledgeable-painted-guarantee.glitch.me/penelitian/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
...
export default InputPeneletian;
