...
const InputPenelitian = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const postResponse = await fetch("http://localhost:5000/penelitian", {
        method: "POST",
        headers: {
  ...
  const handleDeleteData = async (id) => {
    try {
      const deleteResponse = await fetch(`http://localhost:5000/penelitian/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
